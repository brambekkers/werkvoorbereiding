	...
	<div class="content">
		...
		<div class="row">
			<Status
				...
				:key="`card_${key}`"
				:status="typeof item.data === 'boolean' ? item.data : item.data.length"
				:text="item.text"
				:time="item.time"
			/>
		</div>
		...
		<div class="row">
			...
			<div class="col-9">
				<div class="card">
					...
					<div class="card-body">
						<template v-for="(problems, key) in problems.data">
							<b-card
								:title="key"
								:sub-title="`${problems.length} problemen`"
								:key="key"
							>
								<b-card
									v-for="(user, index) of problems"
									:key="user.id"
									class="my-1"
									border-variant="secondary"
									:header="`${index} - ${user.id}`"
...
export default {
	name: "AdminEdit",
	components: { Status, ConnectionButton },
	data() {
		return {
			services: {
				server: {
