							...
							<div class="row">
								...
								<div class="col-md-6">
									...
									<div class="input-group mb-2">
										...
										<input type="password" placeholder="Herhaal wachtwoord" v-model="passwordCheck" required="required" class="form-control">
									</div>
								</div>
							</div>
						</div>
		...
		methods: {
			register() {
				...
				if (this.password === this.passwordCheck) {
					this.$store.state.firebase.fb.auth().createUserWithEmailAndPassword(this.email, this.password)
						.then(() => {
							that.$store.state.firebase.fb.auth().currentUser.sendEmailVerification();
