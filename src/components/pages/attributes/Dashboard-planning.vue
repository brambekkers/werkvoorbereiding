						...
						for (const substap of this.planningStappenArray) {
							...
							while(staptijd > 0){ 
								...
								if(berekenTijd.resttijd === 0){
									...
									array[num].push({stap: substap, tijd: Number(staptijd.toFixed(0))})
									...
									let num = array.length-1
									array[num].push({stap: substap, tijd: Number(beschikbaretijd.toFixed(0))})
		...
		methods: {
			...
			berekenStappenPerDag(beschikbaretijd, staptijd){
						...
						if(staptijd <= beschikbaretijd){
							...
							return {resttijd: 0, beschikbaretijd: beschikbaretijd}
						}
						// Staptijd
						else{
							let resttijd = staptijd -= beschikbaretijd
							return {resttijd: resttijd, beschikbaretijd: 0}
						}
			},
			weekNummer(nummer){
				let num = nummer
